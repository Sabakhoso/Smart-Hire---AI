"""
Prompt template for candidate-to-job matching using Groq.
...
Compare the candidate's resume against the job posting and evaluate
...
def build_matcher_prompt(resume_text: str, job_description: str) -> list[dict]:
    """
    Construct chat messages for Groq with system prompt, job posting, and resume.

    Args:
        resume_text: Extracted plain text from the candidate's resume.
        job_description: The job posting details to match against.
